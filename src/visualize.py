...
def visualize_generated_images(model_output: torch.tensor, x_test: torch.tensor, y_test: torch.tensor):
    originals = []
    recons = []
    labels = []
...
if __name__ == "__main__":
    ...
    auto_encoder = AutoEncoder(
        input_message_dim = config.input_dim, # Flattening the last two dim from x_train; (1, 28, 28) --> (1, 28 * 28)
    ...
    model_output = auto_encoder(x_test)
    visualize_generated_images(model_output, x_test, y_test)